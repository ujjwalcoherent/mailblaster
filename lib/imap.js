...
function plainTextPart(structure) {
  ...
  if (!structure.childNodes) {
    return structure.type === 'text/html'
      ? { part: null, html: true, encoding: structure.encoding }
      : { part: '1', encoding: structure.encoding };
  }

  const plain = findPart(structure, 'text/plain');
  if (plain) return { part: plain.part, encoding: plain.encoding };
  const html = findPart(structure, 'text/html');
  if (html) return { part: html.part, html: true, encoding: html.encoding };
  return null;
}
...
/**
 * Decode a MIME part's raw wire bytes per its own Content-Transfer-Encoding
 * before treating it as text.
 *
 * bodyParts hands back the part exactly as it sat on the wire — for a
 * 7bit/8bit part that already IS the text, but base64 and quoted-printable
 * are common for any HTML email with real formatting (Gmail's own web
 * compose included), and decoding was missing entirely: a base64 HTML part
 * was being run straight through `.toString('utf8')`, which just returns
 * the base64 alphabet itself. That reached the database as a send's stored
 * body, read back in the UI's "View" as a wall of base64 rather than the
 * email — and fed the importer's own template-diffing off literal base64
 * text instead of the actual reconstructed message.
 */
function decodePart(buf, encoding) {
  const enc = String(encoding || '').toLowerCase();
  if (enc === 'base64') {
    try { return Buffer.from(buf.toString('ascii'), 'base64'); } catch (e) { return buf; }
  }
  if (enc === 'quoted-printable') {
    try { return libqp.decode(buf); } catch (e) { return buf; }
  }
  return buf;
}
...
const textOf = (msg, partInfo) => {
  ...
  if (!buf) return '';
  const raw = decodePart(buf, partInfo.encoding).toString('utf8');
...
function buildSentSearch(opts) {
  ...
  /* `to` accepts a single address (string) or several (array) — several
     recipients is "find a campaign sent to ANY of these people," an OR
     across `to` criteria, which ImapFlow compiles into a binary OR tree on
     its own (confirmed directly against its search-compiler.js: an `or`
     array of any length works, not just two). A lone address stays a plain
     `to` criterion rather than a needless one-item OR. */
  const toList = Array.isArray(to) ? to.filter(Boolean) : (to ? [to] : []);
  if (toList.length === 1) search.to = toList[0];
  else if (toList.length > 1) search.or = toList.map(addr => ({ to: addr }));

  /* `query` (subject-or-body) needs its own OR pair. When `to` ALSO needed
     an OR (2+ recipients), IMAP only has one OR per nesting level in this
     shape — expressing "any of these recipients AND (subject OR body)"
     needs the recipient OR nested one level inside an AND with the
     subject/body OR, not two independent top-level `or` keys (the second
     would silently overwrite the first). ImapFlow's own `not`/`or` nesting
     lets a SearchObject value hold another full SearchObject, so the
     recipient OR becomes one branch of a two-item outer OR structure is
     the wrong shape for AND — instead this composes it as two returned
     criteria joined by IMAP's own implicit AND (a SearchObject's sibling
     keys already AND together), by keeping the query OR as `search.or` and
     folding the recipient OR into each branch of it when both are present. */
  if (query) {
    if (toList.length > 1) {
      search.or = toList.map(addr => ({ to: addr, or: [{ subject: query }, { body: query }] }));
    } else {
      search.or = [{ subject: query }, { body: query }];
    }
  }

  if (!since && !until && !subject && !toList.length && !query) search.all = true;
  return search;
}
