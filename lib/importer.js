 ...
 */
/* A pasted subject line is short — one sentence someone typed on purpose.
   A pasted FULL EMAIL BODY (the actual message content, not just a subject)
   is usually much longer and reads as prose across several lines, not one
   short line. Treating the latter as if it were a subject means searching
   for an exact subject match on text that was never a subject at all — it
   would almost never match anything, or coincidentally mismatch. This is
   the threshold used to decide which one a headerless paste actually is. */
const LOOKS_LIKE_BODY_NOT_SUBJECT = 160;
...
function parsePasted(raw) {
  ...
  const parsed = {
    ...
    date: grab('Date'),
    /* A body-text fragment for a wider subject-OR-body search, set only
       when there's no real subject/Message-Id to search on exactly —
       api/import.js's scan() passes this into `query` rather than
       `subject`, so it's matched against the body too, not forced into an
       exact-subject search it was never meant for. */
    bodyQuery: null,
  };
  parsed.to = parsed.to ? parsed.to[0] : null;

  /* No headers at all: this paste is either a short subject line typed by
     hand, or the actual body of a long email with no headers pasted along
     with it. A short first line reads as a subject; several lines or one
     long line reads as body text to search on instead — never guessed at
     as if it were the subject. */
  if (!parsed.subject && !parsed.messageId) {
    const lines = text.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    const first = lines[0];
    if (first) {
      if (lines.length > 1 || first.length > LOOKS_LIKE_BODY_NOT_SUBJECT) {
        // A meaningful fragment for a body search — the invariant OPENING
        // of the message tends to survive across recipients better than a
        // random middle slice (personalised greetings vary; the rest of a
        // template usually doesn't), so this takes the start, not a random
        // window, and caps it well under IMAP's own practical search-term
        // length limits.
        parsed.bodyQuery = lines.join(' ').slice(0, 200);
      } else {
        parsed.subject = first.slice(0, 200);
        parsed.weak = true;
      }
    }
  }
  parsed.usable = !!(parsed.messageId || parsed.subject || parsed.bodyQuery);
  return parsed;
}
