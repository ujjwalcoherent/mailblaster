...
async function insert(e) {
  ...
  if (driver === 'postgres') {
    try {
      await sql`INSERT INTO sends (campaign_id, recipient_id, time, sender, recipient, name, subject,
                                   attachments, status, error, body, message_id,
                                   followup_round, in_reply_to_send, imported)
                VALUES (${e.campaignId || null}, ${recipientId}, ${e.time}, ${e.from || ''}, ${e.to || ''},
                        ...
                        ${round}, ${e.inReplyToSend || null}, ${!!e.imported})`;
    } catch (err) {
  ...
  try {
    db.prepare(`INSERT INTO sends (campaign_id, recipient_id, time, sender, recipient, name, subject,
                                   attachments, status, error, body, message_id,
                                   followup_round, in_reply_to_send, imported)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
      .run(e.campaignId || null, recipientId, e.time, e.from || '', e.to || '', e.name || '',
           ...
           round, e.inReplyToSend || null, e.imported ? 1 : 0);
  } catch (err) {
    if (isDuplicateSend(err)) return 'duplicate';
    throw err;
  }
...
module.exports = {
  ...
  startCampaign, finishCampaign, campaigns,
