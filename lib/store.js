...
function initSqlite() {
  ...
  handle.exec(`CREATE TABLE IF NOT EXISTS sends (
    ...
    attachments TEXT, status TEXT, error TEXT
  )`);
...
async function insert(e) {
  await init();
  if (driver === 'postgres') {
    await sql`INSERT INTO sends (time, sender, recipient, name, subject, attachments, status, error)
...
async function list(limit) {
  ...
  if (driver === 'postgres') {
    const { rows } = await sql`SELECT time, sender, recipient, name, subject, attachments, status, error
    ...
    return rows.map(r => ROW({
      ...
      attachments: r.attachments, status: r.status, error: r.error,
    }));
  }
  if (driver === 'sqlite') {
